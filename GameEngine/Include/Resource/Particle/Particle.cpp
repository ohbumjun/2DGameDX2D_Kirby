...
bool CParticle::AddStructuredBuffer(const std::string& Name, unsigned int Size, unsigned int Count,
	...
	int Register, bool Dynamic, int StructuredBufferShaderType)
{
	...
	for (size_t i = 0; i < BufferCount; ++i)
	{
		...
		{
			if (!m_vecStructuredBuffer[i]->Init(Name, Register, Size, Dynamic, Count, StructuredBufferShaderType))
				return false;

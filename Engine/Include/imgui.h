...
namespace ImGui
{
	...
	IMGUI_API bool InputScalar(const char* label, ImGuiDataType data_type, void* p_data, const void* p_step = nullptr,
	...
	IMGUI_API bool MenuItem(const char* label, const char* shortcut = nullptr, bool selected = false,
	...
	IMGUI_API ImGuiID DockSpaceOverViewport(const ImGuiViewport*    viewport = nullptr, ImGuiDockNodeFlags flags = 0,
	                                        const ImGuiWindowClass* window_class = nullptr);
	...
	IMGUI_API void
	RenderPlatformWindowsDefault(void* platform_render_arg = nullptr, void* renderer_render_arg = nullptr);
...
struct ImVector
{
	...
	ImVector<T>& operator=(const ImVector<T>& src)
	{
		clear();
		resize(src.Size);
		memcpy(Data, src.Data, static_cast<size_t>(Size) * sizeof(T));
		return *this;
	}
	...
	T* erase(const T* it)
	{
		...
		memmove(Data + off, Data + off + 1, (static_cast<size_t>(Size) - static_cast<size_t>(off) - 1) * sizeof(T));
		Size--;
		return Data + off;
	}
	...
	T* erase(const T* it, const T* it_last)
	{
		...
		memmove(Data + off, Data + off + count,
		        (static_cast<size_t>(Size) - static_cast<size_t>(off) - static_cast<size_t>(count)) * sizeof(T));
...
struct ImColor
{
	...
	{
		float sc = 1.0f / 255.0f;
		Value.x  = static_cast<float>((rgba >> IM_COL32_R_SHIFT) & 0xFF) * sc;
		Value.y  = static_cast<float>((rgba >> IM_COL32_G_SHIFT) & 0xFF) * sc;
		Value.z  = static_cast<float>((rgba >> IM_COL32_B_SHIFT) & 0xFF) * sc;
...
struct ImFontAtlas
{
	...
	IMGUI_API ImFont* AddFontFromMemoryCompressedTTF(const void*    compressed_font_data, int compressed_font_size,
	                                                 float          size_pixels, const ImFontConfig* font_cfg = nullptr,
	                                                 const ImWchar* glyph_ranges = nullptr);
...
struct ImFont
{
	...
	{
		return (static_cast<int>(c) < IndexAdvanceX.Size) ? IndexAdvanceX[static_cast<int>(c)] : FallbackAdvanceX;
	}
	...
	IMGUI_API ImVec2 CalcTextSizeA(float size, float max_width, float wrap_width, const char* text_begin,
	                               const char* text_end = nullptr, const char** remaining = nullptr) const; // utf8
...
struct ImGuiPlatformIO
{
	...
	// . F . . .  // (Optional) Set IME (Input Method Editor, e.g. for Asian languages) input position, so text preview appears over the imgui input box. FIXME: The call timing of this is inconsistent because we want to support without multi-viewports.
...
namespace ImGui
{
	...
	static inline void
	OpenPopupContextItem(const char* str_id = nullptr, ImGuiMouseButton mb = 1) { OpenPopupOnItemClick(str_id, mb); }
